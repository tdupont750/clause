...
- Prompts: `prompt_ynq`/`prompt_yes` honor `-y`/`-n`; destructive actions go through `confirm_yes` (typed `yes`; `-y` deliberately does not auto-confirm these, `-n` declines). The reset commands instead ask `prompt_reset_item` (y/n/q) once per file or key, which is what lets them skip the typed confirmation: nothing is written before you see its name, and `q` stops the rest.
