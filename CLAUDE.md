...
- `profile reset <name>` restores clause's shipped configuration: `reseed_profile` overwrites every template file except the ones `preserved_on_reset` carves out (`.claude.json`, `.gitconfig`, `.claude/clause-sudo.log`), so the profile keeps its login, history, git identity and sudo log and loses only local edits to the files clause ships. `reset_files` is the shared list, so the confirmation preview is exactly what gets written; a typed `yes` is required (`-y` does not auto-confirm), and it reminds you to rebuild, plus warns when the `nested` marker survives a Containerfile whose block is commented out again.
