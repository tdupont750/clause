...
- Bootstrap is lazy: `bootstrap_state` idempotently seeds `~/.clause/profiles/default/` before any non-read-only command. The read-only commands (`status`, `profile list`, `config list`, `-h`) never touch disk.
...
- Args resolve via `resolve_args` (raw mode: first line verbatim, a present file wins even if empty): `-a` one-shot, then workspace `.clause/args`, then profile `args`, then (default profile only, when the profile file is absent) the repo `default/args` template. `config reset --local args` deletes the workspace file so args fall through; `config reset args` rewrites the profile file with the template value (never deletes: profile `args`/`effort`/`model` are launch-required files); `config set args ''` writes a present-but-empty profile file meaning no args (`config set --local args ''` for the workspace tier).
...
- Input is validated at parse time before any side effects: `validate_effort`, `validate_model`, `validate_mount_path`, `validate_config_key`, `validate_profile_name`.
...
- Read-only allowlists: `bootstrap_state` skips seeding for `status|profile-list` and `config list`; `detect_runtime` only runs for `build|delete-profile|podman-reset|launch` (everything else must work on a runtime-less host).
