...
- **One command per invocation** — parsing routes every command flag through `set_command` into a single `COMMAND` variable (default `launch`); a second command flag is a parse-time error naming both flags as typed, raised before any side effects. Command bodies live in `cmd_*` functions dispatched from a `case` at the bottom of the script, and `SKIP_BOOTSTRAP` / `NEEDS_RUNTIME` derive from `COMMAND` via small `case` allowlists. Modifiers (`-a`, `-t`, `-w`, `-y`, `-n`, positional profile) combine freely with any command
