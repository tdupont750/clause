...
- `defaults/set-bg.sh` — terminal background-color hook seeded into `<profile>/.claude/hooks/` on first use (invoked by the `defaults/settings.json` hooks)
...
- **`defaults/` — seed files** — `settings.json`, `CLAUDE.md`, `Containerfile`, `clause-args`, and `set-bg.sh` (into `.claude/hooks/`, made executable) are seeded into the profile on first use if missing; never overwritten by the script afterward (users can freely modify their profile's copies); deleted only when the profile is deleted
