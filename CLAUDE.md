...
./clause config [--local] <key> [value]     # keys: args, effort, mount (mount needs --local)
./clause config [--local] --reset <key>
...
- Args resolve via `resolve_args` (raw mode: first line verbatim, a present file wins even if empty): `-a` one-shot, then workspace `.clause/args`, then profile `args`, then (default profile only, when the profile file is absent) the repo `default/args` template. `config --local --reset args` deletes the workspace file so args fall through; `config --reset args` rewrites the profile file with the template value (never deletes: profile `args`/`effort` are launch-required files); `config args ''` writes a present-but-empty profile file meaning no args (`config --local args ''` for the workspace tier).
